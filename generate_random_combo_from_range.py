...
def build_combo_pool(available_indexes):
...
for _ in range(COMBOS_TO_GENERATE):
    combo_pool = build_combo_pool(range_weight_dict)

    random_combo = combo_pool[random.randint(0, len(combo_pool) - 1)]
    range_weight_dict.pop(random_combo)

    print(commbo_dict[random_combo][random.randint(0, len(commbo_dict[random_combo]) - 1)])